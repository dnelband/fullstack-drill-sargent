...
export function fetchOrders(status: OrderStatus | "all" = "all") {
  const query = new URLSearchParams({ status });
  return requestJson<OrderRecord[]>(`/api/orders?${query}`);
}

export function fetchOrderSummary() {
  return requestJson<OrderSummary>("/api/orders/summary");
}
