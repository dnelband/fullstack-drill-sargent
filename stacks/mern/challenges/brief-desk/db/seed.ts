...
import type { BriefPriority, BriefStatus, Member } from "../exercise/types.ts";
...
export async function seedChallenge(db: Db): Promise<void> {
  ...
  const memberName = new Map(members.map((m) => [m._id, m.display_name]));

  await db.collection<Member>("members").insertMany(members);
  await db.collection("briefs").insertMany(
    briefs.map((brief) => ({
      ...brief,
      assigned_member_name: brief.assigned_member_id
        ? (memberName.get(brief.assigned_member_id) ?? null)
        : null,
      version: brief.status === "open" ? 1 : brief.status === "claimed" ? 2 : 3,
      updated_at: now,
    })),
  );
  console.log(`[db] seeded ${members.length} members, ${briefs.length} briefs`);
}
