...
Mutations return a **complete `PageRecord`** on success (`200`). Conflicts return **`409` with `latest`** (full page). Slug collisions also include **`conflicting_page`** (`_id`, `title`, `slug`, `status`). The UI must not invent page fields from message-only errors.
...
**Version:** every successful PATCH / publish / unpublish increments `version`. Filters must be atomic: `{ _id, version: expected_version }` (publish/unpublish also constrain `status` as documented).
